...
Fuel 9 provides an experimental support for builtin NFV features (including
native DPDK and hugepages support). The default Fuel setup does not enable
these (potentially unstable) features that will conflict with this version of
the 6WIND Virtual Accelerator plugin.
For this reason make sure to keep the default Fuel setup that does not use
NFV features.
